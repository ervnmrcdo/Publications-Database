...
const AwardsPageContent: FC = () => {
  const { step, setStep, setDraftId, setDraftUrls, draftsMap, setDraftsMap, setChecklist, setDriveChecklist, setDriveUrl } = useAwardsFlow();
    ...
    if (selectedAward) {
      try {
        ...
        if (draft && (draft.form41 || draft.form42 || draft.form43 || draft.form44)) {
          ...
          setDriveChecklist(draft.driveChecklist || []);
          setDriveUrl(draft.drive_url || '');
        } else {
          setDraftId(null);
          setDraftUrls({});
          setChecklist([]);
