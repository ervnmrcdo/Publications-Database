...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (existingDraft) {
      const filePath = `${existingDraft.submission_id}_form44.pdf`;
    ...
    try {
      ...
      try {
        form.getTextField('title-book.undefined').setText(publication.title || '');
      } catch (e) {
    ...
    const initialDraftPath = `${user_id}_${publicationId}_${awardId}_form44.pdf`;
    try {
